...
| `LAF-78` | low | `RS-12` live acceptance | `open` | — `registry scaffold` writes `artifact.json` and a starter payload and stops, so every setup recipe is hand-written and its shape learned through four successive refusals. Each refusal names what is missing, which is why this is `low`: the maintainer is slowed, never misled |
