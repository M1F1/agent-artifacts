...
| `LAF-84` | low | fixing `LAF-73` | `open` | — a completed `receipt undo` leaves the record's status reading `skipped`, which in this vocabulary means *setup did not run*; there is no status for *ran and was reversed* |
| `LAF-85` | medium | fixing `LAF-73` | `open` | — something wrote to the real user data root at `23:34`–`23:36` on `2026-08-15`, inside this unattended run, while every live scenario ran under a sandbox `HOME`; `make integration`, `unit`, `validate` and `packaging-check` were each measured afterwards and touched nothing there, so the writer is unidentified |
