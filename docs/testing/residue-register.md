...
| `LAF-82` | medium | fixing `LAF-69` | `open` | — this file's opening paragraph says `docs_check` *fails when a document names a finding this file does not carry*; nothing implements that. A checked document may name `LAF-99` and claim any disposition for it, and every rule stays silent because `rows.get` returns nothing |
| `LAF-83` | medium | fixing `LAF-69` | `open` | — `DOC010` reads disposition cells, not sentences, so `release-checklist-v14.md`'s *`LAF-61` becomes visible* — the other half of the case that produced `LAF-69` — still passes. Closing it means constraining how a checked document may mention a finding at all, which is a decision about writing rather than a bug fix |
