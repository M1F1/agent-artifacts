...
first run without blocking the read-only wizard itself. A reviewed source-management operation may
synchronize and persist one policy-allowed required alias at a time, so organizations can onboard
several required sources incrementally. That narrow exception never authorizes content: marketplace,
install/update/setup, and other content-capable operations still fail closed until every required
alias is enabled. Git-host, repository-prefix, direct-source, default-registry, and reporting
constraints remain enforced during source management. Loading does not create files implicitly.

A content request with no enabled source fails with `no-source-configured`. An agent can configure
one through `aart source add --help` and inspect the compiled result with
`aart marketplace list --json`; a human can alternatively use **Add** in the TUI Sources stage.
The legacy `list/install/update/setup` commands remain explicit 0.1 compatibility paths until the
canonical lifecycle CLI surface is delivered. Help/version/local-status/uninstall-class callers set
`content_required=false` and therefore do not need a source or registry.
