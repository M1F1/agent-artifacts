...
class LegacyArtifactAdapterTest(unittest.TestCase):
    def test_maps_current_artifact_shapes_without_importing_legacy_into_protocol(self):
        from agent_artifacts.adapters.legacy_artifact import artifact_manifest_from_legacy
        from agent_artifacts.model import Artifact, Compatibility
        ...
        legacy = Artifact(
            ...
            "code-review",
            "skills/code-review",
            Compatibility(("tabnine", "claude")),
            "Review changes before they merge.",
        )

        manifest = _unwrap(artifact_manifest_from_legacy(legacy, version))

        self.assertEqual(str(manifest.identity), "skill/code-review")
        self.assertEqual(manifest.compatibility.profiles, ("claude", "tabnine"))
        self.assertEqual(manifest.payload.format, "aart-skill-v1")
        self.assertEqual(manifest.install.modes, ("copy", "symlink"))

    def test_rejects_incomplete_legacy_metadata(self):
        from agent_artifacts.adapters.legacy_artifact import artifact_manifest_from_legacy
        from agent_artifacts.model import Artifact, Compatibility
        from agent_artifacts.protocol.semver import parse_semver
        ...
        self.assertEqual(
            _codes(
                artifact_manifest_from_legacy(Artifact("mcp", "postgres", "mcp/postgres"), version)
            ),
            ("artifact-invalid",),
        )
        invalid = (
            Artifact("skill", "Bad_Name", "skills/bad", description="Bad name."),
            Artifact(
                "skill",
                "review",
                "skills/review",
        ...
        for artifact in invalid:
            with self.subTest(artifact=artifact):
                self.assertEqual(
                    _codes(artifact_manifest_from_legacy(artifact, version)),
                    ("artifact-invalid",),
                )

    def test_maps_legacy_setup_and_default_compatibility(self):
        from agent_artifacts.adapters.legacy_artifact import artifact_manifest_from_legacy
        from agent_artifacts.model import Artifact, SetupInstaller
        from agent_artifacts.protocol.semver import parse_semver
        ...
        manifest = _unwrap(artifact_manifest_from_legacy(artifact, version))
        self.assertEqual(
            manifest.compatibility.profiles,
            ("claude", "opencode", "tabnine", "vibe"),
        )
        assert manifest.setup is not None
        self.assertEqual(str(manifest.setup.recipe), "setup/installer.json")

        invalid_setup = Artifact(
            "mcp",
            "atlassian",
            "mcp/atlassian",
            description="Connect to Atlassian.",
        ...
        self.assertEqual(
            _codes(artifact_manifest_from_legacy(invalid_setup, version)),
            ("artifact-invalid",),
        )
