...
run_aart install --bundle base --profile claude --dry-run

# 4. Install specific artifacts
echo -e "\n${GREEN}[4/11] Simulating User: Installing house rules into Claude...${NC}"
run_aart install house --profile claude --yes
...
run_aart install python-style --profile tabnine --yes
...
run_aart install python-style --profile tabnine --force --yes
if grep -q "Drift test!" .tabnine/guidelines/python-style.md; then
...
run_aart install --bundle backend --profile claude --yes
